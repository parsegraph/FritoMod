--[[
-- Event dispatchers provide a generic implementation of the Observer design pattern.
--]]

-- A mixin that provides event dispatching functionality.
--
-- class
--     the class that is the target of this mixin. If class is falsy, then
--     a new EventDispatcher is returned.
-- returns
--     a method used during mixin creation or a new EventDispatcher if class is falsy.
function EventDispatcher(class)
    -- For standalone event dispatchers, both of these would be falsy.
    ...
    function Constructor(instance)
        ...
        if staticConnectors then
            for eventName, connectors in pairs(staticConnectors) do
                connectorTable[eventName] = Lists.Clone(connectors);
            end;
        end;
        ...
        function instance:SetEventInitializer(eventName, initializerFunc, ...)
            initializers[eventName] = Curry(initializerFunc, ...);
        end;
        ...
        -- Notice that the listenerFunc has already been Curry'd, so no extra arguments will be given.
        function instance:AddEventConnector(eventName, connectorFunc, ...)
            ...
            table.insert(connectors, Curry(connectorFunc, ...));
        end;
        ...
        function instance:AddForwarder(forwarderFunc, ...)
            forwarderFunc = Curry(forwarderFunc, ...);
        ...
        function instance:AddListener(eventName, listenerFunc, ...)
            ...
            local listenerFunc = Curry(listenerFunc, ...);
            ...
            local connectors = connectorTable[eventName];
            if connectors then
                Lists.MapCall(connectors, self, eventName, listenerFunc);
            end;
            ...
            return function()
                ...
                Lists.RemoveAll(listeners, listenerFunc);
    ...
    if not class then
        -- If class is falsy, we create a standalone EventDispatcher instance.
        local instance = {};
    ...
    class.AddStaticEventConnector = function(eventName, connectorFunc, ...)
        connectorFunc = Curry(connectorFunc, ...);
