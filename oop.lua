...
-----------------------------------------------------------------------
--
--  The Inheritance Metatable
--
-----------------------------------------------------------------------
--
-- This metatable implements classical inheritance by iterating up the 
-- class's __super attribute. Notice that since nil values are equivalent 
-- to non-existent values, they indicate that the parent's value should be
-- used instead. In short, use false if you wish to have a falsy value 
-- that masks the parent's.
--
...
OOP.Class = function(parentClass, ...)
    ...
    class = {
        ...
        -- A safe reference to the parentClass. You should use this instead of 
        -- getmetatable since that exposes implementation details of our inheritance.
        --
        -- class.__super.__super.__super will return the great-grandparent of that 
        -- class.
        __super = parentClass,
        ...
        -- Add an initializer to this class. These are called immediately before 
        -- this class is used or accessed. An initializer should expect the 
        -- signature initializer(class), where the class's state may be temporarily
        -- unusable, so every effort should be made for an initializer to not use 
        -- the class when it is initialized.
        --  
        -- This function will throw if it is called anytime past the start of 
        -- initialization.
        ...
        -- Add a constructor to this class. These are called before __Init's are 
        -- called on the instance. They should expect the signature constructor(instance).
        --  
        -- This function will throw if it is called after the class has been initialized. 
        -- It is safe to use during initialization.
        ...
        -- Calls all initializers added either on the OOP.Class call, or separately through 
        -- __AddInitializer.
        --
        -- This function is idempotent, and will return silently and perform no action if 
        -- the class is already initialized.
        --
        -- You should never need to call this function - it is called whenever the class's 
        -- attributes are accessed or when the class is used to construct a new instance. 
        -- You may safely call this function at any time.
        ...
        -- Uninitialized is true until initializers are used on this class. That 
        -- occurs either when this class, or a subclass of it, is constructed, or 
        -- _any_ non-internal value on this class is accessed.
        --  
        -- __Initialize sets this value falsy when it is first called, NOT when it 
        -- is completed. Once this value is falsy, __AddInitializer will throw if 
        -- used and calls to __Initialize will return immediately.
        __uninitialized = true,

        -- Unfinalized is true until all initializers have been fired. This occurs 
        -- at the end of a call to __Initialize. Once this value is falsy, 
        -- __AddConstructor will throw if used. It is guaranteed that if this is 
        -- false, then __uninitialized is also false.
        __unfinalized = true,

        -- Internal list of initializers. You should _never_ assume this exists or 
        -- attempt to use it directly. Initializers are only added through 
        -- __AddInitializer, and are not allowed to be removed once added.
    ...
    setmetatable(class, {
        ...
        -- When the class is called, a new instance is created. The process occurs 
        -- in this order:
        --
        -- 1. Create the instance, assigning the metatable to it to implement 
        -- classical inheritance.
        -- 2. Iterate through the inheritance chain, starting with the deepest 
        -- ancestor, and:
        --  a. Initialize it if necessary.
        --  b. Call all constructors on it using our new instance.
        -- 3. Once every constructor has been called, call the class's __Init 
        -- using our new object and any arguments given with the initial call.
        ...
        -- This metamethod ensures that initialization occurs before any non-
        -- internal methods or attributes used.
        __index = function(self, key)
